...
const OperationsDashboard = () => {
  ...
  const acknowledge = async (alertId) => {
    ...
    try {
      ...
      setError(
        requestError?.response?.data?.error || "The alert could not be acknowledged."
      );
    } finally {
  ...
  if (loading) {
    return (
      <Box sx={{ minHeight: "60vh", display: "grid", placeItems: "center" }}>
        <CircularProgress aria-label="Loading operations overview" />
      </Box>
    );
  }
      ...
      <Stack spacing={3}>
        ...
        >
          <Box>
            ...
            <Typography variant="h4">What needs attention now?</Typography>
            <Typography color="text.secondary" sx={{ mt: 0.5 }}>
              Verify new alerts and check which cameras have a recent analytics signal.
            </Typography>
          </Box>
          <IconButton
              ...
              <CardContent>
                ...
                {overview.alerts.length === 0 ? (
                  <Alert severity="success" variant="outlined">
                    No alerts yet. New structured detection events will appear here.
                  </Alert>
                ) : (
                  <Stack spacing={1.5}>
                          ...
                          <Stack spacing={0.75}>
                            <Stack direction="row" spacing={1} flexWrap="wrap">
                              <Chip
                                ...
                                color={severityColor[alert.event.severity] || "default"}
                                size="small"
                              />
                            ...
                            <Typography variant="body2" color="text.secondary">
                              {alert.event.camera_name || `Camera ${alert.event.camera_id}`}
                              {" · "}
